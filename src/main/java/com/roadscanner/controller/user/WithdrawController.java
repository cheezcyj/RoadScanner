...
public class WithdrawController {
    ...
    @ResponseBody
    public String delete(MemberVO user,
                         @SessionAttribute("user") MemberVO authenticatedAdministrator) throws SQLException {
        LOG.debug("┌────────────────────────────────────────────────────────┐");
        LOG.debug("│ WithdrawController delete()                          │");
        LOG.debug("└────────────────────────────────────────────────────────┘");
        
        MessageVO message = new MessageVO();
        if (isInvalidAdministratorTarget(user, authenticatedAdministrator)) {
            message.setMsgId("20");
            message.setMsgContents("회원 탈퇴에 실패했습니다.");
    ...
    @ResponseBody
    public String forbidden(MemberVO user,
                            @SessionAttribute("user") MemberVO authenticatedAdministrator) throws SQLException {
        LOG.debug("┌────────────────────────────────────────────────────────┐");
    ...
    @ResponseBody
    public String clear(MemberVO user,
                        @SessionAttribute("user") MemberVO authenticatedAdministrator) throws SQLException {
        LOG.debug("┌────────────────────────────────────────────────────────┐");
    ...
    private boolean isInvalidAdministratorTarget(MemberVO target, MemberVO authenticatedAdministrator) {
        return target == null
                || target.getId() == null
                || target.getId().trim().isEmpty()
                || authenticatedAdministrator == null
                || target.getId().equals(authenticatedAdministrator.getId());
    }
