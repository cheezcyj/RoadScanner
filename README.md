...
도로 위 장면에서 교통표지판 후보를 검출하고 독일 GTSRB 43종 범위에서 분류하는 Spring MVC 기반 웹 애플리케이션입니다. 신뢰 기준을 통과하지 못하거나 지원 범위 밖인 이미지는 `인식 불가`로 처리합니다. 사진 선택부터 미리보기, 분석 결과 확인과 의견 제출까지 하나의 흐름으로 제공하며 Q&A, 비공개 문의와 관리자 운영 기능을 함께 지원합니다.
...
아래 PowerShell 명령은 저장소 루트에서 실행합니다. 웹 애플리케이션에는 JDK 8과 Maven이 필요합니다. Python 고정 의존성은 64비트 Python 3.12에서 검증했으며, 실제 모델과 LFS 미디어를 복원하려면 Git LFS가 필요합니다.

실제 계정, 이메일, 호스트, 버킷, API 주소와 자격증명을 저장소 파일에 기록하지 않습니다. 필요한 값은 OS·IDE·컨테이너 환경변수 또는 배포 비밀 저장소로 주입합니다. 이 프로젝트는 `.env` 파일을 자동으로 읽지 않으며 `.env.example`은 실제 값을 넣어 커밋하는 파일이 아닌 주요 변수의 참고용 예시입니다. `.env`, 로그, IDE 설정, 빌드 결과물과 키 파일은 Git 추적 대상에서 제외합니다.
