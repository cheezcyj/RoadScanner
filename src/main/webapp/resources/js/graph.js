const graphContextElement = document.querySelector('meta[name="application-context"]');
const graphContextPath = graphContextElement ? graphContextElement.getAttribute('content') : '';
const graphUrl = function (path) {
  return graphContextPath + path;
};

/* 누적 그래프, 표 */
	$.ajax({
	type: "GET",
	url: graphUrl("/totalFeedback"),
	async: true,
	dataType:"json",
	data:{ 
	},
	success:function(data){//통신 성공
	  ...
	  new Chart(document.getElementById("feedback_barchart"), {
	    ...
	    let total = parseInt(data.u1, 10) + parseInt(data.u2, 10) + parseInt(data.u3, 10);
	    $("#total").text(total + "건");
	    
	},
	error:function(){//실패시 처리
	  console.error("Feedback summary request failed");
	    }
	  }); // ajax
...
/* 셀렉트박스, 월별 그래프 */
	...
	let formattedDate = currentYear + '-' + currentMonth.toString().padStart(2, "0") + '-01';
	let dateList = []; // 날짜 리스트
	let u1List = []; // 오류1 리스트
	let u2List = []; // 오류2 리스트
	let u3List = []; // 오류3 리스트
	...
	function defaultMonthlyGraph(uploadDate) {
	    $.ajax({
		    ...
		    error:function(){//실패시 처리
		      console.error("Monthly feedback request failed");
		    }
	    }); // ajax            
	} // defaultMonthlyGraph
